...
% 画像読み込み
img_G = imread(tifFile);
if ndims(img_G) == 3, img_G = rgb2gray(img_G); end
img_G = double(img_G);
[H, W] = size(img_G);
...
% 可視化準備
figure('Color','w');
imagesc(img_G); axis image off; colormap gray; set(gca,'YDir','reverse'); hold on;
...
if numel(x) < 3 || numel(y) < 3
    ...
    % --- 追加：boundary 失敗チェック ---
    if numel(px) < 2 || numel(py) < 2
        warning('ROI %d の外周ポリゴンが取得できません（スキップ）。', r);
        continue;
    end
