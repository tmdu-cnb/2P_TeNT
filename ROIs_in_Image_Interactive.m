...
% コントロールパネル（右側）- ピクセル単位で確実に配置
% ROI番号入力フィールドのラベル
control_x_pos = 1200;  % 2枚表示の場合の右側位置
uicontrol('Parent', fig, ...
          'Style', 'text', ...
          'String', 'ROI番号（カンマ区切り）:', ...
          'Position', [control_x_pos, 720, 300, 25], ...
          'Units', 'pixels', ...
...
% ROI番号入力フィールド
edit_roi = uicontrol('Parent', fig, ...
                     'Style', 'edit', ...
                     'String', '', ...
                     'Position', [control_x_pos, 690, 300, 30], ...
                     'Units', 'pixels', ...
                     'FontSize', 11, ...
                     'BackgroundColor', 'w', ...
                     'HorizontalAlignment', 'left');
...
% 表示ボタン
btn_display = uicontrol('Parent', fig, ...
                        'Style', 'pushbutton', ...
                        'String', '表示', ...
                        'Position', [control_x_pos, 640, 140, 40], ...
                        'Units', 'pixels', ...
                        'FontSize', 12, ...
                        'FontWeight', 'bold', ...
...
% クリアボタン
btn_clear = uicontrol('Parent', fig, ...
                      'Style', 'pushbutton', ...
                      'String', 'クリア', ...
                      'Position', [control_x_pos + 150, 640, 140, 40], ...
                      'Units', 'pixels', ...
                      'FontSize', 12, ...
                      'Callback', @(~,~) clearDisplay());

% 波形表示ボタン
btn_trace = uicontrol('Parent', fig, ...
                      'Style', 'pushbutton', ...
                      'String', '波形を表示', ...
                      'Position', [control_x_pos, 580, 300, 40], ...
                      'Units', 'pixels', ...
                      'FontSize', 12, ...
                      'Callback', @(~,~) showTraces());

% ステータステキスト（初期状態をCSVファイルの状態に応じて設定）
if exist(csvFile, 'file') == 2
    status_msg = 'ROI番号を入力して「表示」をクリック';
else
    status_msg = sprintf('CSVファイルが見つかりません。波形表示にはCSVが必要です。');
end

txt_status = uicontrol('Parent', fig, ...
                       'Style', 'text', ...
                       'String', status_msg, ...
                       'Position', [control_x_pos, 540, 300, 50], ...
                       'Units', 'pixels', ...
                       'FontSize', 10, ...
